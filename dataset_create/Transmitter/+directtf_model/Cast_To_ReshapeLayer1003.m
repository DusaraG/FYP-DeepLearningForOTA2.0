classdef Cast_To_ReshapeLayer1003 < nnet.layer.Layer & nnet.layer.Formattable
    ...
    methods
        function this = Cast_To_ReshapeLayer1003(name, onnxParams)
            this.Name = name;
            this.NumInputs = 2;
            this.OutputNames = {'model_Conv1D_1_Con_4'};
            this.ONNXParams = onnxParams;
            this.const_fold_opt__188 = onnxParams.Learnables.const_fold_opt__188;
            this.model_Embedding_embe = onnxParams.Learnables.model_Embedding_embe;
        end
        
        function [model_Conv1D_1_Con_4] = predict(this, input_2, input_2NumDims)
            if isdlarray(input_2)
                input_2 = stripdims(input_2);
            end
            input_2NumDims = numel(input_2NumDims);
            onnxParams = this.ONNXParams;
            onnxParams.Learnables.const_fold_opt__188 = this.const_fold_opt__188;
            onnxParams.Learnables.model_Embedding_embe = this.model_Embedding_embe;
            [model_Conv1D_1_Con_4, model_Conv1D_1_Con_4NumDims] = Cast_To_ReshapeFcn(input_2, input_2NumDims, onnxParams, 'Training', false, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {model_Conv1D_1_Con_4}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Cast_To_ReshapeLayer1003');
                error(message('nnet_cnn_onnx:onnx:BadCustomLayerRuntimeOutput', 'Cast_To_ReshapeLayer1003'));
            end
            model_Conv1D_1_Con_4 = dlarray(single(model_Conv1D_1_Con_4), 'SSCB');
            if ~coder.target('MATLAB')
                model_Conv1D_1_Con_4 = extractdata(model_Conv1D_1_Con_4);
            end
        end
        
        function [model_Conv1D_1_Con_4] = forward(this, input_2, input_2NumDims)
            if isdlarray(input_2)
                input_2 = stripdims(input_2);
            end
            input_2NumDims = numel(input_2NumDims);
            ...
            onnxParams.Learnables.const_fold_opt__188 = this.const_fold_opt__188;
            onnxParams.Learnables.model_Embedding_embe = this.model_Embedding_embe;
            [model_Conv1D_1_Con_4, model_Conv1D_1_Con_4NumDims] = Cast_To_ReshapeFcn(input_2, input_2NumDims, onnxParams, 'Training', true, ...
                'InputDataPermutation', {['as-is'], ['as-is']}, ...
                'OutputDataPermutation', {[3 4 2 1], ['as-is']});
            if any(cellfun(@(A)~isnumeric(A) && ~islogical(A), {model_Conv1D_1_Con_4}))
                fprintf('Runtime error in network. At least one output of custom layer ''%s'' is a non-numeric, non-logical value.\n', 'Cast_To_ReshapeLayer1003');
...
function [model_Conv1D_1_Con_4, model_Conv1D_1_Con_4NumDims, state] = Cast_To_ReshapeFcn(input_2, input_2NumDims, params, varargin)
%CAST_TO_RESHAPEFCN Function implementing an imported ONNX network.
%
...
%
% [MODEL_CONV1D_1_CON_4] = Cast_To_ReshapeFcn(INPUT_2, PARAMS)
%			- Evaluates the imported ONNX network CAST_TO_RESHAPEFCN with input(s)
%			INPUT_2 and the imported network parameters in PARAMS. Returns
%			network output(s) in MODEL_CONV1D_1_CON_4.
%
% [MODEL_CONV1D_1_CON_4, STATE] = Cast_To_ReshapeFcn(INPUT_2, PARAMS)
%			- Additionally returns state variables in STATE. When training,
%			use this form and set TRAINING to true.
%
% [__] = Cast_To_ReshapeFcn(INPUT_2, PARAMS, 'NAME1', VAL1, 'NAME2', VAL2, ...)
...
% Outputs:
% --------
% MODEL_CONV1D_1_CON_4
%			- Output(s) of the ONNX network.
%			  Without permutation, the size(s) of the outputs are:
%				  MODEL_CONV1D_1_CON_4:		[Unknown, Unknown, Unknown, Unknown]				Type: FLOAT
...
[Vars, NumDims] = packageVariables(params, {'input_2'}, {input_2}, [input_2NumDims]);
% Call the top-level graph function:
[model_Conv1D_1_Con_4, model_Conv1D_1_Con_4NumDims, state] = Cast_To_ReshapeGraph1000(input_2, NumDims.input_2, Vars, NumDims, Training, params.State);
% Postprocess the output data
[model_Conv1D_1_Con_4] = postprocessOutput(model_Conv1D_1_Con_4, outputDataPerms, anyDlarrayInputs, Training, varargin{:});
end

function [model_Conv1D_1_Con_4, model_Conv1D_1_Con_4NumDims1002, state] = Cast_To_ReshapeGraph1000(input_2, input_2NumDims1001, Vars, NumDims, Training, state)
% Function implementing the graph 'Cast_To_ReshapeGraph1000'
% Update Vars and NumDims from the graph's formal input parameters. Note that state variables are already in Vars.
Vars.input_2 = input_2;
NumDims.input_2 = input_2NumDims1001;

% Execute the operators:
% Cast:
Vars.model_Embedding_Cast = cast(int32(extractdata(Vars.input_2)), 'like', Vars.input_2);
NumDims.model_Embedding_Cast = NumDims.input_2;

% Gather:
[Vars.model_Embedding_em_1, NumDims.model_Embedding_em_1] = onnxGather(Vars.model_Embedding_embe, Vars.model_Embedding_Cast, 0, NumDims.model_Embedding_embe, NumDims.model_Embedding_Cast);

% Reshape:
[shape, NumDims.model_reshape_5_Resh] = prepareReshapeArgs(Vars.model_Embedding_em_1, Vars.new_shape__172, NumDims.model_Embedding_em_1, 0);
Vars.model_reshape_5_Resh = reshape(Vars.model_Embedding_em_1, shape{:});

% Unsqueeze:
[shape, NumDims.model_Conv1D_1_Conv1] = prepareUnsqueezeArgs(Vars.model_reshape_5_Resh, Vars.const_fold_opt__188, NumDims.model_reshape_5_Resh);
Vars.model_Conv1D_1_Conv1 = reshape(Vars.model_reshape_5_Resh, shape);

% Reshape:
[shape, NumDims.model_Conv1D_1_Con_4] = prepareReshapeArgs(Vars.model_Conv1D_1_Conv1, Vars.new_shape__168, NumDims.model_Conv1D_1_Conv1, 0);
Vars.model_Conv1D_1_Con_4 = reshape(Vars.model_Conv1D_1_Conv1, shape{:});

% Set graph output arguments from Vars and NumDims:
model_Conv1D_1_Con_4 = Vars.model_Conv1D_1_Con_4;
model_Conv1D_1_Con_4NumDims1002 = NumDims.model_Conv1D_1_Con_4;
% Set output state from Vars:
state = updateStruct(state, Vars);
end

function [inputDataPerms, outputDataPerms, Training] = parseInputs(input_2, numDataOutputs, params, varargin)
% Function to validate inputs to Cast_To_ReshapeFcn:
p = inputParser;
...
function [model_Conv1D_1_Con_4] = postprocessOutput(model_Conv1D_1_Con_4, outputDataPerms, anyDlarrayInputs, Training, varargin)
% Set output type:
if ~anyDlarrayInputs && ~Training
    if isdlarray(model_Conv1D_1_Con_4)
        model_Conv1D_1_Con_4 = extractdata(model_Conv1D_1_Con_4);
    end
end
% Permute outputs if requested:
model_Conv1D_1_Con_4 = permuteOutputVar(model_Conv1D_1_Con_4, outputDataPerms{1}, 4);
end
